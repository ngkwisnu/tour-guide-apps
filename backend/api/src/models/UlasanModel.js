...
const addUlasan = (body) => {
    const { rating, ulasan, keunggulan, file, id_user, id_wisata, id_paket, created_at, updated_at } = body;
    console.log(body);
    const SQLQuery = `
        INSERT INTO ulasan (rating, ulasan, keunggulan, file, id_user, id_wisata, id_paket, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const values = [ rating, ulasan, keunggulan, file, id_user, id_wisata, id_paket, created_at, updated_at ];
...
const updateUlasan = (body, id) => {
    ...
    const SQLQuery = `
        UPDATE ulasan 
        SET rating = ?, ulasan = ?, keunggulan = ?, file = ?, id_user = ?, id_wisata = ?, id_paket = ?, created_at = ?, updated_at = ?
        WHERE id = ?
    `;
    const values =  [ rating, ulasan, keunggulan, file, id_user, id_wisata, id_paket, created_at, updated_at, id ];
    return dbPool.execute(SQLQuery, values);
}
