  ...
  const renderBg = () => {
    return (
      <div className="hidden md:block">
        ...
        <Image className="absolute bottom-[100px] right-full mr-40" src={clientSay2} alt="client 2" />
          ...
          >
            <div className={`relative whitespace-nowrap overflow-hidden`} {...handlers}>
              <AnimatePresence initial={false} custom={direction}>
                ...
                >
                  <>
                    ...
                    <span className="block mt-8 text-2xl font-semibold">{currentItem.clientName}</span>
              ...
              <div className="mt-10 flex items-center justify-center space-x-2">
                {data.map((item, i) => (
                  <button className={`w-2 h-2 rounded-full ${i === index ? 'bg-black/70' : 'bg-black/10 '}`} onClick={() => changeItemId(i)} key={i} />
                ))}
              </div>
            </div>
          </MotionConfig>
